...
from utils.plant_profile_generator import generate_plant_profile
...
if generate:
    ...
    db_record = find_plant_record(db, plant_name.strip(), scientific_name.strip() or None)

    ref_bytes = [f.getvalue() for f in refs] if refs else []
    plant_profile = generate_plant_profile(
        plant_name.strip(),
    ...
    for idx, label in enumerate(SEASON_LABELS):
        prompt = build_image_prompt(base_prompt, plant_profile, label, season_texts[idx], len(ref_bytes))
    ...
    with open(out_history, "a", encoding="utf-8") as f:
        f.write(json.dumps({"timestamp": ts, "plant": plant_profile, "output": out_png.name}, ensure_ascii=False) + "\n")
